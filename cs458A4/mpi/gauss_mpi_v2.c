...
//    MPI_Barrier(MPI_COMM_WORLD);
    ...
    for (j = i+1; j < nsize; j++) {
     if (i = nsize - 2) printf("proc %d here\n", proc_id);
     MPI_Barrier(MPI_COMM_WORLD);
     if ( j%nprocs == proc_id ) {
        if (i = nsize - 2) printf("proc %d enters the cond\n", proc_id);
        pivotval = matrix[j][i];
        matrix[j][i] = 0.0;
	for (k = i + 1; k < nsize; k++) {
             matrix[j][k] -= pivotval * matrix[i][k];
             if (i = nsize - 2) printf("dsb %d, matrix[%d][%d] = %f\n",proc_id,j,k,matrix[j][k]);
        }
        R[j] -= pivotval * R[i];
      }
      MPI_Barrier(MPI_COMM_WORLD);
